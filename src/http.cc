...
char* HTTPResponseBuilder(int responseCode, char* response, char* type) {
    ...
    if (responseCode == 200) {
        headertoreturn = constructHTTPHeader200(servstring, strlen(response), type);
        datatoreturn = (char*) malloc (strlen(headertoreturn) + strlen(response));
        sprintf(datatoreturn, "%s%s", headertoreturn, response);
    } else if (responseCode == 404) {
        headertoreturn = constructHTTPHeader404(servstring, strlen(ErrorPage404), type);
        ...
        headertoreturn = constructHTTPHeader418(servstring, strlen(Error418), type);
        datatoreturn = (char*) malloc(strlen(headertoreturn) + strlen(Error418));
        sprintf(datatoreturn, "%s%s", headertoreturn, Error418);
    } else {
        headertoreturn = constructHTTPHeader500(servstring, strlen(Error500), type);
...
void connection(int mySocket) {
    ...
    if (strstr(path, "..")) {
        response = HTTPResponseBuilder(403, NULL, "text/html");
    ...
    if (NULL == responeFile) {
        response = HTTPResponseBuilder(404, NULL, "text/html");
        write(mySocket, response, strlen(response));
        close(mySocket);
    } else {
        ...
        if (filebuf) {
            ...
            perror("Critical: Out of memory!");
        }
        fclose (responeFile);

        if(serverPrefrences["allowOctetStreamForUnsupportedContent"]) {
            if (ends_with(path, ".html") || ends_with(path, ".htm") || ends_with(path, ".php") || ends_with(path, "/")) {
                response = HTTPResponseBuilder(200, filebuf, "text/html");
            } else {
                // response = HTTPResponseBuilder(200, filebuf, "application/octet-stream");
                
                char* HTTPHeader = (char*) malloc(strlen(versionString) + 1000);
                sprintf(HTTPHeader, "HTTP/1.1 200 OK\nStatus: 200 OK\nServer: %s\nContent-Type: application/octet-stream\nContent-Disposition: attachment\nContent-Length: %lld\nAccept-Ranges: bytes\nConnection: close\n\n", versionString, length);

                char* datatoreturn = (char*) malloc ((long long)strlen(HTTPHeader) + length);
                sprintf(datatoreturn, "%s%s", HTTPHeader, filebuf);
